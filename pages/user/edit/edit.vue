		...
		<u-form :model="model" :rules="rules" ref="uForm" :errorType="errorType">
			...
			<u-form-item :label-position="labelPosition" label="角色" label-width="150" prop="role">
				<u-checkbox-group @change="checkboxGroupChange" :width="radioCheckWidth" :wrap="radioCheckWrap">
					<u-checkbox 
					v-model="item.checked" v-for="(item, index) in rList" 
...
export default {
	...
	async onLoad(option) {
		...
		Object.assign(this.model,this.option)
		this.model.registration = this.option.register_date
		// 学院列
			...
			this.$refs.uForm.validate(valid => {
				if (valid) {
					...
					uniCloud.callFunction({
						...
						console.info('updateUser Success <<<' , r.result.code)
